...
export default class GameInstance extends Component {
    ...
    //Shuffles the the possible answers to vote on and adds them to the state
    ...
    addResultsInfo( resultsInfo ) {
        if (resultsInfo) {
            //Sorts the results by score - high to low.
    ...
    //Sets an interval every second to do a get from the database and update all the states
    gameSync() {
        let isActive = true;
        if(isActive)
            {
    ...
    //Conditionally renders the state depending on the game and round states while passing through the information each component needs
    gameState() {
        //If the game state is complete show the final results
        ...
        //If the findGame boolean has been set to true, render the component depending on the roundState switch
    ...
    render() {
        return (
            <div className="row justify-content-center">
                {/*Calls the gameState function to render the appropirate component*/}  
                {this.gameState()}
            </div>
        )
    }
